...
/**
 * Base class that represents a row from the 'pathway' table.
 *
 ...
 */
abstract class BasePathway extends BaseObject  implements Persistent {


  const PEER = 'PathwayPeer';
	...
	/**
	 * Initializes internal state of BasePathway object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Pathway The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = PathwayPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = PathwayPeer::NAME;
		}
	...
	public function setSchoolYearId($v)
	{
		...
		if ($this->school_year_id !== $v) {
			...
			$this->modifiedColumns[] = PathwayPeer::SCHOOL_YEAR_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = PathwayPeer::NUM_COLUMNS - PathwayPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Pathway object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PathwayPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BasePathway:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePathway:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePathway:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePathway:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			PathwayPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PathwayPeer::doInsert($this, $con);
					...
					$affectedRows += PathwayPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PathwayPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PathwayPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = PathwayPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PathwayPeer::ID)) $criteria->add(PathwayPeer::ID, $this->id);
		if ($this->isColumnModified(PathwayPeer::NAME)) $criteria->add(PathwayPeer::NAME, $this->name);
		if ($this->isColumnModified(PathwayPeer::SCHOOL_YEAR_ID)) $criteria->add(PathwayPeer::SCHOOL_YEAR_ID, $this->school_year_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PathwayPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Pathway (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Pathway Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getSchoolYear(PropelPDO $con = null)
	{
		if ($this->aSchoolYear === null && ($this->school_year_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aSchoolYear->addPathways($this);
			 */
		}
	 ...
	 * Otherwise if this Pathway has previously been saved, it will retrieve
	 * related PathwayStudents from storage. If this Pathway is new, it will return
		...
		if ($this->collPathwayStudents === null) {
			if ($this->isNew()) {
			   $this->collPathwayStudents = array();
				...
				$criteria->add(PathwayStudentPeer::PATHWAY_ID, $this->id);
	 ...
	 * Otherwise if this Pathway is new, it will return
	 * an empty collection; or if this Pathway has previously
	 * been saved, it will retrieve related PathwayStudents from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Pathway.
	 */
	public function getPathwayStudentsJoinStudent($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PathwayPeer::DATABASE_NAME);
		}
		...
		if ($this->collPathwayStudents === null) {
			if ($this->isNew()) {
				...
				$this->collPathwayStudents = PathwayStudentPeer::doSelectJoinStudent($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePathway:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePathway::%s', $method));
    }
