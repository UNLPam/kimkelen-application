...
/**
 * Base class that represents a row from the 'course_subject_teacher' table.
 *
 ...
 */
abstract class BaseCourseSubjectTeacher extends BaseObject  implements Persistent {


  const PEER = 'CourseSubjectTeacherPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        CourseSubjectTeacherPeer
	 */
	...
	/**
	 * The value for the situation_r_id field.
	 * @var        int
	 */
	protected $situation_r_id;
	...
	/**
	 * Initializes internal state of BaseCourseSubjectTeacher object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [situation_r_id] column value.
	 * Referencia la situación de revista del profesor en la materia actual
	 * @return     int
	 */
	public function getSituationRId()
	{
		return $this->situation_r_id;
	}
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = CourseSubjectTeacherPeer::ID;
		}
	...
	public function setTeacherId($v)
	{
		...
		if ($this->teacher_id !== $v) {
			$this->teacher_id = $v;
			$this->modifiedColumns[] = CourseSubjectTeacherPeer::TEACHER_ID;
		}
	...
	public function setCourseSubjectId($v)
	{
		...
		if ($this->course_subject_id !== $v) {
			...
			$this->modifiedColumns[] = CourseSubjectTeacherPeer::COURSE_SUBJECT_ID;
		}
	...
	/**
	 * Set the value of [situation_r_id] column.
	 * Referencia la situación de revista del profesor en la materia actual
	 * @param      int $v new value
	 * @return     CourseSubjectTeacher The current object (for fluent API support)
	 */
	public function setSituationRId($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->situation_r_id !== $v) {
			$this->situation_r_id = $v;
			$this->modifiedColumns[] = CourseSubjectTeacherPeer::SITUATION_R_ID;
		}

		if ($this->aSituation_R !== null && $this->aSituation_R->getId() !== $v) {
	...
	public function setDateFrom($v)
	{
		...
		if ( $this->date_from !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = CourseSubjectTeacherPeer::DATE_FROM;
			}
	...
	public function setDateTo($v)
	{
		...
		if ( $this->date_to !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = CourseSubjectTeacherPeer::DATE_TO;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->situation_r_id = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->date_from = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->date_to = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->resetModified();
			...
			return $startcol + 6; // 6 = CourseSubjectTeacherPeer::NUM_COLUMNS - CourseSubjectTeacherPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating CourseSubjectTeacher object", $e);
		}
	}
	...
	public function ensureConsistency()
	{
		...
		if ($this->aSituation_R !== null && $this->situation_r_id !== $this->aSituation_R->getId()) {
			$this->aSituation_R = null;
		}
	} // ensureConsistency
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = CourseSubjectTeacherPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseCourseSubjectTeacher:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			CourseSubjectTeacherPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseCourseSubjectTeacher:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseCourseSubjectTeacher:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseCourseSubjectTeacher:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			CourseSubjectTeacherPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aSituation_R !== null) {
				if ($this->aSituation_R->isModified() || $this->aSituation_R->isNew()) {
					$affectedRows += $this->aSituation_R->save($con);
				}
				$this->setSituation_R($this->aSituation_R);
			}
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CourseSubjectTeacherPeer::doInsert($this, $con);
					...
					$affectedRows += CourseSubjectTeacherPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aSituation_R !== null) {
				if (!$this->aSituation_R->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aSituation_R->getValidationFailures());
				}
			}


			if (($retval = CourseSubjectTeacherPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = CourseSubjectTeacherPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = CourseSubjectTeacherPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getTeacherId(),
			$keys[2] => $this->getCourseSubjectId(),
			$keys[3] => $this->getSituationRId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setSituationRId($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setDateFrom($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setDateTo($arr[$keys[5]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CourseSubjectTeacherPeer::ID)) $criteria->add(CourseSubjectTeacherPeer::ID, $this->id);
		if ($this->isColumnModified(CourseSubjectTeacherPeer::TEACHER_ID)) $criteria->add(CourseSubjectTeacherPeer::TEACHER_ID, $this->teacher_id);
		if ($this->isColumnModified(CourseSubjectTeacherPeer::COURSE_SUBJECT_ID)) $criteria->add(CourseSubjectTeacherPeer::COURSE_SUBJECT_ID, $this->course_subject_id);
		if ($this->isColumnModified(CourseSubjectTeacherPeer::SITUATION_R_ID)) $criteria->add(CourseSubjectTeacherPeer::SITUATION_R_ID, $this->situation_r_id);
		if ($this->isColumnModified(CourseSubjectTeacherPeer::DATE_FROM)) $criteria->add(CourseSubjectTeacherPeer::DATE_FROM, $this->date_from);
		if ($this->isColumnModified(CourseSubjectTeacherPeer::DATE_TO)) $criteria->add(CourseSubjectTeacherPeer::DATE_TO, $this->date_to);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CourseSubjectTeacherPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of CourseSubjectTeacher (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setSituationRId($this->situation_r_id);
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     CourseSubjectTeacher Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     CourseSubjectTeacherPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new CourseSubjectTeacherPeer();
		}
		return self::$peer;
	}
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseCourseSubjectTeacher:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseCourseSubjectTeacher::%s', $method));
    }
