...
/**
 * Base class that represents a row from the 'student_tag' table.
 *
 * 
 *
 ...
 */
abstract class BaseStudentTag extends BaseObject  implements Persistent {


  const PEER = 'StudentTagPeer';
	...
	/**
	 * Initializes internal state of BaseStudentTag object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     StudentTag The current object (for fluent API support)
	 */
	public function setStudentId($v)
	{
		...
		if ($this->student_id !== $v) {
			$this->student_id = $v;
			$this->modifiedColumns[] = StudentTagPeer::STUDENT_ID;
		}
	...
	public function setTagId($v)
	{
		...
		if ($this->tag_id !== $v) {
			$this->tag_id = $v;
			$this->modifiedColumns[] = StudentTagPeer::TAG_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = StudentTagPeer::NUM_COLUMNS - StudentTagPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating StudentTag object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = StudentTagPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseStudentTag:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			StudentTagPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseStudentTag:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseStudentTag:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseStudentTag:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			StudentTagPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = StudentTagPeer::doInsert($this, $con);
					...
					$affectedRows += StudentTagPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = StudentTagPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = StudentTagPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = StudentTagPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(StudentTagPeer::STUDENT_ID)) $criteria->add(StudentTagPeer::STUDENT_ID, $this->student_id);
		if ($this->isColumnModified(StudentTagPeer::TAG_ID)) $criteria->add(StudentTagPeer::TAG_ID, $this->tag_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(StudentTagPeer::STUDENT_ID, $this->student_id);
		$criteria->add(StudentTagPeer::TAG_ID, $this->tag_id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of StudentTag (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     StudentTag Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     StudentTagPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new StudentTagPeer();
		}
		return self::$peer;
	}
	...
	public function getStudent(PropelPDO $con = null)
	{
		if ($this->aStudent === null && ($this->student_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aStudent->addStudentTags($this);
			 */
		}
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseStudentTag:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseStudentTag::%s', $method));
    }
