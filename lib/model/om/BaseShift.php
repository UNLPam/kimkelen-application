 ...
 */
abstract class BaseShift extends BaseObject  implements Persistent {


  const PEER = 'ShiftPeer';
	...
	/**
	 * Initializes internal state of BaseShift object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Shift The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = ShiftPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = ShiftPeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = ShiftPeer::NUM_COLUMNS - ShiftPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Shift object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ShiftPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseShift:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseShift:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseShift:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseShift:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			ShiftPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = ShiftPeer::doInsert($this, $con);
					...
					$affectedRows += ShiftPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ShiftPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ShiftPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = ShiftPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ShiftPeer::ID)) $criteria->add(ShiftPeer::ID, $this->id);
		if ($this->isColumnModified(ShiftPeer::NAME)) $criteria->add(ShiftPeer::NAME, $this->name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ShiftPeer::ID, $this->id);
	 ...
	 * Otherwise if this Shift has previously been saved, it will retrieve
	 * related SchoolYearStudents from storage. If this Shift is new, it will return
		...
		if ($this->collSchoolYearStudents === null) {
			if ($this->isNew()) {
			   $this->collSchoolYearStudents = array();
				...
				$criteria->add(SchoolYearStudentPeer::SHIFT_ID, $this->id);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Shift has previously been saved, it will retrieve
	 * related Divisions from storage. If this Shift is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collDivisions === null) {
			if ($this->isNew()) {
			   $this->collDivisions = array();
				...
				$criteria->add(DivisionPeer::SHIFT_ID, $this->id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseShift:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseShift::%s', $method));
    }
