...
/**
 * Base class that represents a row from the 'occupation' table.
 *
 * 
 *
 ...
 */
abstract class BaseOccupation extends BaseObject  implements Persistent {


  const PEER = 'OccupationPeer';
	...
	/**
	 * Initializes internal state of BaseOccupation object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Occupation The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = OccupationPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = OccupationPeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = OccupationPeer::NUM_COLUMNS - OccupationPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Occupation object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = OccupationPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseOccupation:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			OccupationPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseOccupation:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOccupation:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseOccupation:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			OccupationPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = OccupationPeer::doInsert($this, $con);
					...
					$affectedRows += OccupationPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = OccupationPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = OccupationPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = OccupationPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(OccupationPeer::ID)) $criteria->add(OccupationPeer::ID, $this->id);
		if ($this->isColumnModified(OccupationPeer::NAME)) $criteria->add(OccupationPeer::NAME, $this->name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(OccupationPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Occupation (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Occupation Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     OccupationPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new OccupationPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this Occupation has previously been saved, it will retrieve
	 * related Students from storage. If this Occupation is new, it will return
		...
		if ($this->collStudents === null) {
			if ($this->isNew()) {
			   $this->collStudents = array();
				...
				$criteria->add(StudentPeer::OCCUPATION_ID, $this->id);
	 ...
	 * Otherwise if this Occupation is new, it will return
	 * an empty collection; or if this Occupation has previously
	 * been saved, it will retrieve related Students from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Occupation.
	 */
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Occupation has previously been saved, it will retrieve
	 * related Tutors from storage. If this Occupation is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collTutors === null) {
			if ($this->isNew()) {
			   $this->collTutors = array();
				...
				$criteria->add(TutorPeer::OCCUPATION_ID, $this->id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseOccupation:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseOccupation::%s', $method));
    }
