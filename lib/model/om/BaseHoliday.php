...
abstract class BaseHoliday extends BaseObject  implements Persistent {
  ...
  const PEER = 'HolidayPeer';
	...
	/**
	 * Initializes internal state of BaseHoliday object.
	 * @see        applyDefaults()
	 */
	...
	public function setDay($v)
	{
		...
		if ( $this->day !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = HolidayPeer::DAY;
			}
	...
	public function setDescription($v)
	{
		...
		if ($this->description !== $v) {
			$this->description = $v;
			$this->modifiedColumns[] = HolidayPeer::DESCRIPTION;
		}
    ...
    foreach (sfMixer::getCallables('BaseHoliday:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseHoliday:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseHoliday:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseHoliday:save:post') as $callable)
    {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(HolidayPeer::DAY)) $criteria->add(HolidayPeer::DAY, $this->day);
		if ($this->isColumnModified(HolidayPeer::DESCRIPTION)) $criteria->add(HolidayPeer::DESCRIPTION, $this->description);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseHoliday:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseHoliday::%s', $method));
    }
