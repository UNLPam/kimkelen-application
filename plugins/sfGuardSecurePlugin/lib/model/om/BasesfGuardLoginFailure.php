...
/**
 * Base class that represents a row from the 'sf_guard_secure_login_failure' table.
 *
 * 
 *
...
abstract class BasesfGuardLoginFailure extends BaseObject  implements Persistent {
  ...
  const PEER = 'sfGuardLoginFailurePeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        sfGuardLoginFailurePeer
	 */
	...
	/**
	 * Initializes internal state of BasesfGuardLoginFailure object.
	 * @see        applyDefaults()
	 */
	...
	public function getFailedAt($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->failed_at === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->failed_at);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->failed_at, true), $x);
			}
		}
	 ...
	 * @return     sfGuardLoginFailure The current object (for fluent API support)
	 */
	...
	public function setFailedAt($v)
	{
		...
		if ( $this->failed_at !== null || $dt !== null ) {
			...
			$currNorm = ($this->failed_at !== null && $tmpDt = new DateTime($this->failed_at)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->failed_at = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = sfGuardLoginFailurePeer::FAILED_AT;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->cookie_id = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->failed_at = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			...
			return $startcol + 5; // 5 = sfGuardLoginFailurePeer::NUM_COLUMNS - sfGuardLoginFailurePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = sfGuardLoginFailurePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BasesfGuardLoginFailure:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
			...
			sfGuardLoginFailurePeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setFailedAt($arr[$keys[3]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(sfGuardLoginFailurePeer::FAILED_AT)) $criteria->add(sfGuardLoginFailurePeer::FAILED_AT, $this->failed_at);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of sfGuardLoginFailure (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     sfGuardLoginFailure Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     sfGuardLoginFailurePeer
	 */
