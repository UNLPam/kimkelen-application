...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
  ...
  const PEER = 'sfGuardUserPeer';
	...
	/**
	 * The value for the change_password_at field.
	 * @var        string
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [change_password_at] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	...
	{
		if ($this->change_password_at === null) {
			return null;
		}


		if ($this->change_password_at === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->change_password_at);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->change_password_at, true), $x);
			}
		}
	...
	/**
	 * Sets the value of [change_password_at] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     sfGuardUser The current object (for fluent API support)
	...
	public function setChangePasswordAt($v)
	{
		...
		if ( $this->change_password_at !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->change_password_at !== null && $tmpDt = new DateTime($this->change_password_at)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->change_password_at = ($dt ? $dt->format('Y-m-d H:i:s') : null);
	...
	/**
	 * Set the value of [must_change_password] column.
	 * 
	 * @param      boolean $v new value
	 * @return     sfGuardUser The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(sfGuardUserPeer::ALGORITHM,sfGuardUserPeer::IS_ACTIVE,sfGuardUserPeer::IS_SUPER_ADMIN,sfGuardUserPeer::MUST_CHANGE_PASSWORD))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->change_password_at = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->must_change_password = ($row[$startcol + 10] !== null) ? (boolean) $row[$startcol + 10] : null;
			$this->resetModified();
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[9] => $this->getChangePasswordAt(),
			$keys[10] => $this->getMustChangePassword(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[9], $arr)) $this->setChangePasswordAt($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setMustChangePassword($arr[$keys[10]]);
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Persons from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collPersons === null) {
			if ($this->isNew()) {
			   $this->collPersons = array();
				...
				$criteria->add(PersonPeer::USER_ID, $this->id);
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getPersonsJoinAddress($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collPersons === null) {
			if ($this->isNew()) {
				...
				$this->collPersons = PersonPeer::doSelectJoinAddress($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
