...
abstract class BaseState extends BaseObject  implements Persistent {
  ...
  const PEER = 'StatePeer';
	 ...
	 * related Addresss from storage. If this State is new, it will return
		...
		if ($this->collAddresss === null) {
			if ($this->isNew()) {
			   $this->collAddresss = array();
				...
				$criteria->add(AddressPeer::STATE_ID, $this->id);
	 ...
	 */
	public function getAddresssJoinCity($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(StatePeer::DATABASE_NAME);
		}
		...
		if ($this->collAddresss === null) {
			if ($this->isNew()) {
				...
				$this->collAddresss = AddressPeer::doSelectJoinCity($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
